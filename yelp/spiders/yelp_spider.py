...
class YelpSpider(Spider):
    ...
    start_urls = ['https://www.yelp.com/search?find_desc=&find_loc=Chicago%2C+IL&ns=1']
    ...
    def parse(self, response):
        ...
        result_urls = ['https://www.yelp.com/search?find_desc=&find_loc=Chicago%2C+IL&ns=' + str(x) for x in
                       range(0, total, 1)]
    ...
    def parse_result_page(self, response):
        ...
        restaurant_urls_ending = response.xpath('//a[@class="css-1m051bw"]/@href').extract()[1:]
    ...
    def parse_restaurant_page(self, response):
        ...
        text = response.xpath('//span[@class = " css-1fdy0l5"]/text()').extract()
        # print(text)
        total = int(text[0].split(' ')[0])
        # print(total)
    ...
    def parse_restaurant_reviews_page(self, response):
        ...
        # print(reviews_text)
        restaurant = response.xpath('//h1[@class = "css-12dgwvn"]/text()').extract_first()
        # print(restaurant)
        address = response.xpath('//div[@class=" css-qyp8bo"]/text()').extract_first()
